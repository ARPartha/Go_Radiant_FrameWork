...
func NewRouter(e *Radiant) *Router {
	return &Router{
		tree: &node{
...
var value []string 
//Set route->handler according to language config
func (e *Radiant ) Route(path string, handler interface{},method interface{}, m ...MiddlewareFunc) {
	...
	if core.Configure.Server["LanguageRedirect"] == "true" && core.Configure.Server["Swagger"]=="false" {
		...
		re := e.Group("/:lang")
		switch routeController := handler.(type) {
		case func(Context) error:
			if strings.Contains(value[i],"GET"){
				re.rGET(path, routeController)
			}
			if strings.Contains(value[i],"POST"){
				re.rPOST(path, routeController)
			}
			if strings.Contains(value[i],"PUT"){
				re.rPUT(path, routeController)
			}
			if strings.Contains(value[i],"PATCH"){
				re.rPATCH(path, routeController)
			}
		...
		case func(Context) error:
			...
			if strings.Contains(value[i],"GET"){
				e.rGET(path, routeController)
			}
			if strings.Contains(value[i],"POST"){
				e.rPOST(path, routeController)
			}
			if strings.Contains(value[i],"PUT"){
				e.rPUT(path, routeController)
			}
			if strings.Contains(value[i],"PATCH"){
				e.rPATCH(path, routeController)
			}
		case HandlerFunc:
			if strings.Contains(value[i],"GET"){
				e.rGET(path, routeController)
